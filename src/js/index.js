(function($) {
    ...
    var callTree = function(data, config, container) {
        ...
        this.childrenKey = config.children || 'children';
    ...
    callTree.prototype.bindEvent = function() {
        var self = this;
        var treeContainer = this.container.find('.call-tree');
        treeContainer.on('click', '.parent .unexpand', function(e) {
            var target = $(e.target);
            var parent = target.closest('.parent');
            var data = parent.data('children');
            target.removeClass('unexpand').addClass('expand');
            var children = treeContainer.find('[data-pid=' + parent.data('id') + ']');
        ...
        treeContainer.on('click', '.parent .expand', function(e) {
        ...
        treeContainer.on('click', '.row', function(e) {
            var row = $(e.target).closest('.row');
            self.container.find('.row').removeClass('selected');
            row.addClass('selected');
        });

        treeContainer.on('dblclick', '.row', function(e) {
